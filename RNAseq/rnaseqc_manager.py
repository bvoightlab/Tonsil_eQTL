...
for sample in Romp:
	...
	remove = "rm "+sample+".Aligned.sortedByCoord.out.md.bam"
	runstuff(remove)

	
	#assemble rnaseqc command
	markdup = "python /project/voight_ML/lorenzk/Romberg_eQTL/scripts/run_rnaseqc.py /project/voight_ML/lorenzk/Romberg_eQTL/reference_files/gencode.v34.GRCh38.genes.collapsed_only.gtf "+sample+".Aligned.sortedByCoord.out.id.bam "+sample+" --stranded rf"
	
	#submit job
	runstuff("bsub -g /lorenzk/"+Name+" -o "+sample+"rnaseqc_lsf_log.out "+ markdup)
