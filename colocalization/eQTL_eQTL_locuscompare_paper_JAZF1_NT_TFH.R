...
#args <- c("NaiveB", "GCB", "ENSG00000153814.13", "JAZF1")
#args <- c("NaiveT", "TFH", "ENSG00000153814.13", "JAZF1")
...
label <- filter(joint, variant_id == "chr7:28121414:C:G")
...
plot1 <- ggplot(joint, aes(x = -log10(pval_nominal.x), y = -log10(pval_nominal.y))) +
  ...
  ggtitle(chrpos) +
  theme_classic()+
  theme(plot.title = element_text(hjust = 1), legend.title = element_text(hjust = 1), legend.position=c(.9,.7), legend.background = element_rect(fill = "NA"))
