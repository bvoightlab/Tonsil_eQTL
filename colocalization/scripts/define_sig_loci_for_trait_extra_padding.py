...
def main(argv): 
    ...
    stagger_bp = options_dict.get('-t', 100000)
    ...
    file_format = options_dict.get('-n', 'TRAIT.ANC.sumstats_mrmegaP.txt')
...
def driver(gwas_file, output_loc, prime_p_thresh, sec_p_thresh, stagger_bp, chromo_lengths, file_format):
    ...
    sig_tiles = [int(x[0]) + (int(x[1])//stagger_bp)/10000 for x in temp]
    ...
    for i in range(len(sig_tiles_no_dups)):
        ...
        start_bp = int(round(10000*(cur_tile%1)) * stagger_bp) #This gets the start position of the current tile (ending in 0)
        ...
        elif start_bp == (cur_locus[2] + stagger_bp) and chromo == cur_locus[0]: #Extend the current locus
            cur_locus[2] = cur_locus[2] + (stagger_bp*2)
