...
#make M and F splits
metaM <- filter(full_meta, Sex == 1)
metaF <- filter(full_meta, Sex == 2)

mergedM  <- merged[,metaM$header]
mergedF  <- merged[,metaF$header]

#run DESeq2

#import data
DE_data <- DESeqDataSetFromMatrix(countData = mergedM, 
                                  colData = metaM,
...
resultsfile <- str_c("/project/voight_viz/lorenzk/Romberg_eQTL/DESeq2/", trait1stuff[1], "_", trait2stuff[1], "_DESEQ_results_Mpaired.txt")
write.table(res, file= resultsfile, sep="\t", row.names=TRUE, quote=FALSE)

resOrdered <- res[order(res$pvalue),]
resSig <- subset(resOrdered, padj < 0.0025)

resultsfile <- str_c("/project/voight_viz/lorenzk/Romberg_eQTL/DESeq2/", trait1stuff[1], "_", trait2stuff[1], "_DESEQ_results_Mpaired_sig.txt")
write.table(resSig, file= resultsfile, sep="\t", row.names=TRUE, quote=FALSE)

#import data
DE_data <- DESeqDataSetFromMatrix(countData = mergedF, 
                                  colData = metaF,
                                  design = formula(paste("~ sample + celltype")))
...
resultsfile <- str_c("/project/voight_viz/lorenzk/Romberg_eQTL/DESeq2/", trait1stuff[1], "_", trait2stuff[1], "_DESEQ_results_Fpaired.txt")
write.table(res, file= resultsfile, sep="\t", row.names=TRUE, quote=FALSE)

resOrdered <- res[order(res$pvalue),]
resSig <- subset(resOrdered, padj < 0.0025)

resultsfile <- str_c("/project/voight_viz/lorenzk/Romberg_eQTL/DESeq2/", trait1stuff[1], "_", trait2stuff[1], "_DESEQ_results_Fpaired_sig.txt")
write.table(resSig, file= resultsfile, sep="\t", row.names=TRUE, quote=FALSE)
